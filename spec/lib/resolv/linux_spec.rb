...
RSpec.describe Dory::Resolv::Linux do
  ...
  let(:stub_resolv_file) do
    ->(filename = resolv_file) do
      allow(Dory::Resolv::Linux).to receive(:common_resolv_file) { filename }
      allow(Dory::Resolv::Linux).to receive(:ubuntu_resolv_file) { filename }
      # make sure we aren't going to over-write the real resolv file
      expect(Dory::Resolv::Linux.resolv_file).to eq(filename)
    end
  end
  ...
  it 'returns common resolv file on fedora and arch' do
    ...
    expect(Dory::Resolv::Linux.common_resolv_file).to eq(resolv_file)
    set_fedora.call()
    expect(Dory::Resolv::Linux.resolv_file).to eq(resolv_file)
    set_arch.call()
    expect(Dory::Resolv::Linux.resolv_file).to eq(resolv_file)
  end
  ...
  context "resolv file creation/destruction" do
    ...
    before :each do
      ...
      expect(Dory::Resolv::Linux.common_resolv_file).to eq(filename)
      system("rm #{filename}")
      expect(File.exist?(Dory::Resolv::Linux.common_resolv_file)).to be_falsey
    end

    it 'returns common resolv file on unknown platform if it exists' do
      expect{
        system("touch #{filename}")
      }.to change{File.exist?(Dory::Resolv::Linux.common_resolv_file)}.from(false).to(true)
      set_unknown_platform.call()
      expect(Dory::Resolv::Linux.resolv_file).to eq(filename)
      expect{
        system("rm #{filename}")
      }.to change{File.exist?(Dory::Resolv::Linux.common_resolv_file)}.from(true).to(false)
      expect{Dory::Resolv::Linux.resolv_file}.to raise_error(RuntimeError, /unable.*location.*resolv.*file/i)
    end
  end
  ...
  context "editing the file" do
    ...
    it "adds the nameserver when it doesn't exist" do
      file_contents.each do |contents|
        ...
        expect(Dory::Resolv::Linux).not_to have_our_nameserver
        expect{Dory::Resolv::Linux.configure}.to change{Dory::Resolv::Linux.has_our_nameserver?}.from(false).to(true)
        expect(Dory::Resolv::Linux).to have_our_nameserver
      end
    end
    ...
    it "doesn't add the nameserver twice" do
      file_contents.each do |contents|
        ...
        expect{Dory::Resolv::Linux.configure}.to change{Dory::Resolv::Linux.has_our_nameserver?}.from(false).to(true)
        expect(Dory::Resolv::Linux).to have_our_nameserver
        lines = Dory::Resolv::Linux.resolv_file_contents.split("\n")
    ...
    it "cleans up properly" do
      file_contents.each do |contents|
        ...
        expect{Dory::Resolv::Linux.configure}.to change{Dory::Resolv::Linux.has_our_nameserver?}.from(false).to(true)
        expect(Dory::Resolv::Linux).to have_our_nameserver
        expect{Dory::Resolv::Linux.clean}.to change{Dory::Resolv::Linux.has_our_nameserver?}.from(true).to(false)
        expect(Dory::Resolv::Linux).not_to have_our_nameserver
        expect(File.read(resolv_file)).to eq(contents)
        expect(Dory::Resolv::Linux.resolv_file_contents).to eq(contents)
      end
    end
  end
  ...
  context "knows if we've edited the file" do
    ...
    %w[127.0.0.1 192.168.53.164].each do |nameserver|
      ...
      it "doesn't think we edited the file if 127.0.0.1 is there but the comment isn't" do
        ...
        expect(Dory::Resolv::Linux.contents_has_our_nameserver?(contents)).to be_falsey
      end

      it "does think we edited the file if 127.0.0.1 is there and the comment is also" do
        stub_resolv.call(nameserver)
        contents = "nameserver 1.1.1.1\n#{comment}\nnameserver #{nameserver}\nnameserver 2.2.2.2"
        expect(Dory::Resolv::Linux.contents_has_our_nameserver?(contents)).to be_truthy
      end
    end
  end
end
